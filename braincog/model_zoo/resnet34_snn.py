...
class ResNet(BaseModule):
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, groups=1, width_per_group=64,
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(3, self.inplanes, kernel_size=(7, 7), stride=2, padding=3, bias=False),
            self._norm_layer(num_features=self.inplanes, alpha=np.sqrt(.5)),
            self.node(),
            nn.MaxPool2d(2)
        )
...
def resnet34s(pretrained=False, progress=True, **kwargs):
    # print(kwargs['n_preact'])
    if 'n_preact' in kwargs and kwargs['n_preact'] is True:
        return _resnet('react_resnet34s', ReactBlock, [3, 4, 6, 3], pretrained, progress, **kwargs)
    return _resnet('resnet34s', BasicBlock, [3, 4, 6, 3], pretrained, progress, **kwargs)
