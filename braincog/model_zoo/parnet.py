...
def activation_func(activation):
    assert isinstance(activation, object)
...
class SSEBlock(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.conv = nn.Conv2d(
            self.in_channels, self.out_channels, kernel_size=(1, 1))
...
class Downsample(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.conv1 = conv_bn(
            self.in_channels, self.out_channels, conv_sampler(kernel_size=1))
        self.conv2 = conv_bn(self.in_channels, self.out_channels,
                             conv_sampler(kernel_size=3, stride=2))
        self.conv3 = conv_van(
            self.in_channels, self.out_channels, conv_sampler(kernel_size=1))
        self.globalAvgPool = GlobalAveragePool2D()
...
class Fusion(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=(2, 2))
        self.conv1 = conv_bn(self.network_in_channels, self.out_channels, conv_sampler(
        ...
        self.conv3 = conv_van(self.network_in_channels, self.out_channels, conv_sampler(
            kernel_size=1, groups=2))
        self.globalAvgPool = GlobalAveragePool2D()
        self.activation = activation_func('silu')
        self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, input1, input2):
        a = torch.cat([self.bn1(input1), self.bn2(input2)], dim=1)
...
class Stream(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.fuse = nn.Sequential(
            FuseBlock(self.in_channels, self.out_channels))
        self.activation = activation_func('silu')
...
class FuseBlock(nn.Module):
    def __init__(self, in_channels, out_channels) -> None:
        ...
        self.conv1 = conv_bn(
            self.in_channels, self.out_channels, conv_sampler(kernel_size=1))
        self.conv2 = conv_bn(self.in_channels, self.out_channels,
                             conv_sampler(kernel_size=3, stride=1))
...
class ParNetEncoder(nn.Module):
    def __init__(self, in_channels, block_size, depth) -> None:
        ...
        self.stream1_downsample = Downsample(
        ...
        self.stream2_fusion = Fusion(self.block_size[2], self.block_size[2])
...
class ParNetEncoderCifar(nn.Module):
    def __init__(self, in_channels, block_size, depth) -> None:
        super().__init__()
        self.in_channels = in_channels
        self.block_size = block_size
        ...
        self.head = nn.Sequential(
            nn.Conv2d(in_channels, self.block_size[0], kernel_size=(
                3, 3), padding=(1, 1)),
            nn.BatchNorm2d(self.block_size[0]),
            nn.SiLU(),
            nn.Conv2d(self.block_size[0], self.block_size[0],
        ...
        self.d3 = Downsample(self.block_size[2], self.block_size[2])
        self.stream1 = nn.Sequential(
            *[Stream(self.block_size[0], self.block_size[0]) for _ in range(self.depth[0])]
        )
        ...
        self.stream2 = nn.Sequential(
            *[Stream(self.block_size[1], self.block_size[1]) for _ in range(self.depth[1])]
        )

        self.stream3 = nn.Sequential(
            *[Stream(self.block_size[2], self.block_size[2]) for _ in range(self.depth[2])]
        )

        self.stream2_fusion = Fusion(self.block_size[1], self.block_size[2])
        self.stream3_fusion = Fusion(self.block_size[2], self.block_size[2])
...
class ParNetDecoderCifar(nn.Module):
    def __init__(self, in_channels, n_classes) -> None:
        super().__init__()
        self.avg = nn.AdaptiveAvgPool2d((1, 1))
        self.decoder = nn.Linear(in_channels, n_classes)
...
class ParNetCifar(nn.Module):
    def __init__(self, in_channels=3, num_classes=10, block_size=[128, 256, 512], depth=[4, 5, 5], **kwargs) -> None:
        super().__init__()
        self.encoder = ParNetEncoderCifar(in_channels, block_size, depth)
        self.decoder = ParNetDecoderCifar(block_size[-1], num_classes)
...
def parnet_l(in_channels=3, n_classes=10, **kwargs):
