...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        self.stride = stride
        self.node1 = node(channel=planes)
...
class ResNet(BaseModule):
    def __init__(self, block, layers, num_classes=10, zero_init_residual=False, groups=1, width_per_group=64,
        ...
        self.inplanes = 128
        self.dilation = 1
        self.node1 = self.node(channel=self.inplanes)
        self.node2 = self.node(channel=-1)
        ...
        self.conv1 = nn.Conv2d(data_channel * self.init_channel_mul, self.inplanes, kernel_size=3, stride=1, padding=1,
                               bias=False)
...
def resnet19(pretrained=False, progress=True, **kwargs):
    if 'n_preact' in kwargs and kwargs['n_preact'] is True:
        return _resnet('react_resnet19', ReactBlock, [3, 3, 2], pretrained, progress, **kwargs)
